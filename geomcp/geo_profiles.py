...
def _esearch(db: str, term: str, retmax: int = 20) -> dict:
    ...
    if not email:
        print(
            "Warning: No email configured for NCBI E-Utils. Consider adding one for better compliance.",
            file=sys.stderr,
        )
...
def _esummary(db: str, ids: list) -> dict:
    ...
    try:
        resp = requests.get(
            f"{config.get('base_url', 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils')}/esummary.fcgi",
            params=params,
            timeout=30,
        )
        resp.raise_for_status()
    ...
    try:
        ...
        if result["datasets"]:
            formatted_result = {
                ...
                "result": {"uids": [r.get("uid") for r in result["datasets"]]},
            }
...
def search_geo_series(term: str, retmax: int = 20) -> dict:
    ...
    try:
        ...
        if result["series"]:
            formatted_result = {
                "header": {"type": "esummary", "version": "0.3"},
                "result": {"uids": [r.get("uid") for r in result["series"]]},
            }
            # Add each record to the result dict
...
def search_geo_samples(term: str, retmax: int = 20) -> dict:
    ...
    try:
        ...
        if result["samples"]:
            formatted_result = {
                "header": {"type": "esummary", "version": "0.3"},
                "result": {"uids": [r.get("uid") for r in result["samples"]]},
            }
            # Add each record to the result dict
...
def search_geo_platforms(term: str, retmax: int = 20) -> dict:
    ...
    try:
        ...
        if result["platforms"]:
            formatted_result = {
                "header": {"type": "esummary", "version": "0.3"},
                "result": {"uids": [r.get("uid") for r in result["platforms"]]},
            }
            # Add each record to the result dict
